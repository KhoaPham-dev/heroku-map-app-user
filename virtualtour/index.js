...
(async function () {
  ...
  var res = await fetch("/preloadViewVR");
  try {
    data = await res.json();
  } catch (error) {
    console.log(error);
  ...
  let initScene;
  let initNameArr = decodeURIComponent(location.href)
    .split("name=")[1]
    .split('"')[1]
    .split("|");
  for (let i = 0; i < initNameArr.length; i++) {
    initScene = scenes.find((e) => {
      return e.data.name.toLowerCase() == initNameArr[i].toLowerCase();
    });
    if (initScene) break;
  }

  switchScene(initScene ? initScene : scenes[0]);
})();
